...
def m_rungekutta4per_E(func, y_0, t, args={}):
    y = np.zeros([len(t), len(y_0)])
    y[0] = y_0
...
def Potential2(y1,y2,w1,w2,vy1,vy2,vw1,vw2,t):
    ...
    #the scale factor is handled as if it was uniform among all agents
    return np.exp(-0.5*(delta_X/rangei)**2-0.5*(delta_Y/rangei)**2)
...
def force2Xper(y1,y2,w1,w2,vy1,vy2,vw1,vw2,t):
    ...
    delta_Y=min([abs(w1-w2),abs(w1-w2+LY),abs(w1-w2-LY)])
    #the scale factor is handled as if it was uniform among all agents
    return signs_X[np.argmin([abs(y1-y2),abs(y1-y2+LX),abs(y1-y2-LX)])]*(delta_X/rangei)*np.exp(-0.5*(delta_X/rangei)**2-0.5*(delta_Y/rangei)**2)
...
def force2Yper(y1,y2,w1,w2,vy1,vy2,vw1,vw2,t):
    ...
    delta_Y=min([abs(w1-w2),abs(w1-w2+LY),abs(w1-w2-LY)])
    #the scale factor is handled as if it was uniform among all agents
    return signs_Y[np.argmin([abs(w1-w2),abs(w1-w2+LY),abs(w1-w2-LY)])]*(delta_Y/rangei)*np.exp(-0.5*(delta_X/rangei)**2-0.5*(delta_Y/rangei)**2)
...
def Vext(yi,wi,t):
	return 0*( np.cos(yi-t)+np.cos(yi-t))
...
y,Ene = m_rungekutta4per_E(pendulo7, c_i_mult, t, args_sol)
...
xedges = np.arange(0,20,1)
yedges = np.arange(0,20,1)
